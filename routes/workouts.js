...
router.post("/", (req, res) => {
  ...
  const result = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(USER_ID, client_id, date, day_name, split, duration_mins, volume_lbs || 0, rpe, calories, notes);
...
router.put("/:id", (req, res) => {
  ...
  const workout = db.prepare("SELECT id FROM workouts WHERE id = ? AND user_id = ?").get(req.params.id, USER_ID);
